...
export default function Navigation() {
  return (
    <nav
      className="flex px-20 h-16 items-center justify-between backdrop-blur fixed top-0 
    left-0 right-0 z-50 bg-background/50
"
    >
              ...
              <NavigationMenuItem key={menu.name}>
                ...
                <NavigationMenuContent>
                  <ul className="grid w-[500px] font-light gap-3 p-4 grid-cols-2">
                      {menu.items?.map((itme) =>(
                          <NavigationMenuItem key={itme.name}>
                            <NavigationMenuLink>
                              <Link
                                  className="p-3 space-y-1 block leading-none"
                                  to={itme.to}>{itme.name}</Link>
                            </NavigationMenuLink>
                          </NavigationMenuItem>
                      ))}
                  </ul>
